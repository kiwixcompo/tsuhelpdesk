                        ...
                        <div id="viewDetails">
                                        ...
                                        <div class="form-group">
                                            <div class="d-flex justify-content-between align-items-center mb-2">
                                                ...
                                                <div class="d-flex align-items-center" style="gap: 8px;">
                                                    <span id="puterStatusPill" class="badge badge-light border py-1 px-2" style="font-size: 0.72rem; cursor: pointer; border-radius: 20px; font-weight: 600;" title="Click to switch Puter accounts">
                            ...
                            <?php if (isset($_GET['i4cus']) && $_GET['i4cus'] == 1 && ($_SESSION['role_id'] == 5 || $_SESSION['role_id'] == 8)): ?>
                                <form method="post" class="mb-3" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        <label for="feedback" class="font-weight-bold mb-2">Feedback</label>
...
Return ONLY the response text that the support staff should send to the student. Do not write any intro or outro (e.g. do not say "Here is a draft" or "Dear Support"). Just output the exact text to be pasted into the reply box.`;
    ...
    $(document).on('click', '#btnRephraseWithAI', async function() {
        const btn = $(this);
        const originalHtml = btn.html();
        const textarea = $('textarea[name="reply_text"]');
        const currentText = textarea.val().trim();
        
        if (!currentText) {
            alert('Please type some text in the reply box first before rephrasing.');
            return;
        }
        ...
        try {
            const prompt = `You are a helpful university ICT Support assistant.
An ICT staff has drafted a response to a student's complaint. Help rephrase, polish, and refine this draft to make it sound highly professional, exceptionally polite, clear, and grammatically flawless, while keeping all original meaning, key technical details, instructions, and facts exactly the same:

Draft response: "${currentText}"

Return ONLY the professionally rephrased response text that the support staff should send to the student. Do not write any intro or outro (e.g. do not say "Here is the rephrased version:" or "Dear student"). Just output the exact text to be pasted into the reply box.`;
        ...
        if (typeof puter !== 'undefined') {
            function updatePuterPill() {
                getPuterUserWithTimeout(1500).then(function(user) {
                    $('#puterActiveUser').text(user.username);
